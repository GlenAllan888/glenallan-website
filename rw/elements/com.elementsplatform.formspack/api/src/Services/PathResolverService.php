...
class PathResolverService
{
    ...
    public function resolveContentPath(string $relativePath, ?Request $request = null): ?string
    {
        try {
            ...
            // Note: serverRoot already includes subfolder path for subfolder deployments
            if (!$basePath) {
                $basePath = rtrim(self::$resolvedServerRoot, '/');
            }
    ...
    private function calculateContentPathFromReferer(string $refererPath): ?string
    {
        ...
        // Note: serverRoot already includes subfolder path for subfolder deployments (e.g. my-site.com/sub-folder),
        // so we must NOT add siteBasePath again - that would duplicate the subfolder in the path
        $refererBasePath = rtrim(self::$resolvedServerRoot, '/') . $refererDir;
